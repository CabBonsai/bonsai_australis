// app/api/research-project-journal/route.ts
//
// Server-side proxy for `research_project_journal` (RLS enabled, no anon
...
// GET /api/research-project-journal                 -> all rows
// GET /api/research-project-journal?id=x             -> single row
// GET /api/research-project-journal?project_id=x     -> rows for one project
export async function GET(req: NextRequest) {
  const id = req.nextUrl.searchParams.get('id');
  const projectId = req.nextUrl.searchParams.get(PROJECT_FK);

  let query = supabaseServer.from('research_project_journal').select('*');
...
// POST /api/research-project-journal  body: { project_id, ...columns }
...
// PATCH /api/research-project-journal  body: { id, ...fields to update }
...
// DELETE /api/research-project-journal?id=x
